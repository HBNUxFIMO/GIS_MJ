...
# 1단계: NDWI, NDVI, NDMI 데이터 로드 및 좌표 추출
...
def load_ndvi_data(month):
    file_path = f'/Users/seo/Desktop/GIS/GIS_Pro/ndvi/month{month}.tif'
...
def load_ndmi_data(month):
    file_path = f'/Users/seo/Desktop/GIS/GIS_Pro/ndmi/month{month}.tif'
...
# 가뭄 위험 지역 계산 함수
def calculate_drought_risk_areas(ndvi_data, high_threshold, low_threshold, region_size, distance_threshold, nodata_value):
    rows, cols = ndvi_data.shape
    grid_y, grid_x = np.mgrid[0:rows:region_size, 0:cols:region_size]
    
    # NoData 값과 NaN 값 필터링
    valid_mask = (ndvi_data[grid_y, grid_x] != nodata_value) & (~np.isnan(ndvi_data[grid_y, grid_x]))
    
    high_risk_mask = (ndvi_data[grid_y, grid_x] <= high_threshold) & valid_mask
    low_risk_mask = (ndvi_data[grid_y, grid_x] >= low_threshold) & valid_mask
    medium_risk_mask = (ndvi_data[grid_y, grid_x] > high_threshold) & (ndvi_data[grid_y, grid_x] < low_threshold) & valid_mask
...
# 산불 위험 지역 계산 함수
def calculate_fire_risk_areas(ndmi_data, high_threshold, low_threshold, region_size, distance_threshold, nodata_value):
    rows, cols = ndmi_data.shape
    grid_y, grid_x = np.mgrid[0:rows:region_size, 0:cols:region_size]
    
    # NoData 값과 NaN 값 필터링
    valid_mask = (ndmi_data[grid_y, grid_x] != nodata_value) & (~np.isnan(ndmi_data[grid_y, grid_x]))
    
    high_risk_mask = (ndmi_data[grid_y, grid_x] <= high_threshold) & valid_mask
    low_risk_mask = (ndmi_data[grid_y, grid_x] >= low_threshold) & valid_mask
    medium_risk_mask = (ndmi_data[grid_y, grid_x] > high_threshold) & (ndmi_data[grid_y, grid_x] < low_threshold) & valid_mask
...
# 가뭄 위험도 엔드포인트
@app.route('/api/drought-risk/<int:month>/', methods=['GET'])
def get_drought_risk(month):
    ndvi_data, transform_data, src_crs, nodata_value = load_ndvi_data(month)
    high_threshold = -0.5  # 가뭄 위험도에서 높은 위험도를 결정하는 임계값
    low_threshold = 0.15   # 가뭄 위험도에서 낮은 위험도를 결정하는 임계값
    
    high_risk, medium_risk, low_risk = calculate_drought_risk_areas(ndvi_data, high_threshold, low_threshold, region_size, distance_threshold, nodata_value)
...
# 산불 위험도 엔드포인트
@app.route('/api/fire-risk/<int:month>/', methods=['GET'])
def get_fire_risk(month):
    ndmi_data, transform_data, src_crs, nodata_value = load_ndmi_data(month)
    high_threshold = -3  # 산불 위험도에서 높은 위험도를 결정하는 임계값
    low_threshold = 0.1   # 산불 위험도에서 낮은 위험도를 결정하는 임계값
    
    high_risk, medium_risk, low_risk = calculate_fire_risk_areas(ndmi_data, high_threshold, low_threshold, region_size, distance_threshold, nodata_value)
